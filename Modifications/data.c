...
data load_data_captcha(char **paths, int n, int m, int k, int w, int h)
{
    ...
    //Allouer de la mémoire pour les filenames
    d.filenames = (char**)xcalloc(n, sizeof(char*));
    int i;
    for(i = 0; i < n; ++i){
        fill_truth_captcha(paths[i], k, d.y.vals[i]);
        //Enregistre les filenames
        d.filenames[i] = strdup(paths[i]);
    }
    if(m) free(paths);
    return d;
}
...
data load_data_region(int n, char **paths, int m, int w, int h, int size, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        //Enregistre les filenames
        d.filenames[i] = strdup(random_paths[i]);
...
pthread_t load_data_in_thread(load_args args)
{
    ...
    *ptr = args;
    // Allouer et initialiser le tableau filenames
    if (args.n > 0) {
        ptr->d->filenames = (char**)xcalloc(args.n, sizeof(char*));
        for (int i = 0; i < args.n; ++i) {
            ptr->d->filenames[i] = strdup(args.paths[i]);
        }
    }
...
void *load_threads(void *ptr)
{
    ...
    for (i = 0; i < args.threads; ++i) {
        ...
        // Allouer et enregistrer filenames
        if (args.n > 0) {
            args.d->filenames = (char**)xcalloc(args.n, sizeof(char*));
            for (int j = 0; j < args.n; ++j) {
                args.d->filenames[j] = strdup(args.paths[(i * total / args.threads) + j]);
            }
        }
    ...
    float aspect, float hue, float saturation, float exposure, int use_mixup, int use_blur, int show_imgs, float label_smooth_eps, int dontuse_opencv, int contrastive)
{
    ...
    if (use_mixup && rand_int(0, 1)) {
        ...
        //Allouer de la mémoire pour les filenames
	    d2.filenames = (char**)xcalloc(n, sizeof(char*));
	    int i;
        for(i = 0; i<n; ++i){
            //Enregistre les filenames
            d2.filenames[i] = strdup(paths_mix[i]);
        }
        ...
        if (use_mixup >= 3) {
            ...
            //Allouer de la mémoire pour les filenames
	        d3.filenames = (char**)xcalloc(n, sizeof(char*));
            int i;
            for(i = 0; i<n; ++i){
                //Enregistre les filenames
                d3.filenames[i] = strdup(paths_mix3[i]);
            }
            ...
            //Allouer de la mémoire pour les filenames
	        d4.filenames = (char**)xcalloc(n, sizeof(char*));
            for(i = 0; i<n; ++i){
                //Enregistre les filenames
                d4.filenames[i] = strdup(paths_mix4[i]);
            }
...
data concat_data(data d1, data d2)
{
    ...
    //Concaténer filenames
    d.filenames = (char**)xcalloc(d.X.rows, sizeof(char*));
    int i;
    for(i = 0; i < d1.X.rows; ++i){
        d.filenames[i] = d1.filenames[i];
    }
    for(i = 0; i < d2.X.rows; ++i){
        d.filenames[d1.X.rows + i] = d2.filenames[i];
    }    
    return d;
}
...
data load_categorical_data_csv(char *filename, int target, int k)
{
    ...
    d.y = y;
    //Allouer de la mémoire pour les filenames
	d.filenames = (char**)xcalloc(X.rows, sizeof(char*));
    int i;
    for(i=0; i<X.rows; ++i){
        d.filenames[i]=filename;
    }
...
int get_num_patient(const char* name)
{
    char num_patient[5];
    strncpy(num_patient, name, 4);
    num_patient[4]='\0';
    return atoi(num_patient); //conversion en entier pour faciliter les comparaisons
}

int is_in(int nb, int liste[], int len)
{
    int i;
    for(i = 0; i<len; i++){
        if(liste[i]==nb){
...
{
    // On alloue de l'espace pour les espaces de train et test
    data* split = (data*)xcalloc(2, sizeof(data));
    int i;
    int start = part*d.X.rows/total; // indice de début de l'ensemble de test
    int end = (part+1)*d.X.rows/total; // indice de fin de l'ensmeble de test
    data train ={0}; // initialisation d'un ensemble de données de train vide
    data test ={0}; // idem pour les données de test
    train.shallow = test.shallow = 1;

    // Configure les dimensions des ensembles de train et test
    test.X.rows = test.y.rows = end-start;
    train.X.rows = train.y.rows = d.X.rows - (end-start);
    train.X.cols = test.X.cols = d.X.cols;
    train.y.cols = test.y.cols = d.y.cols;

    // Alloue de l'espace pour les matrices de données
    ...
    // On stockera les numéros de patients déjà mis dans le train ou le test
    int * train_nums = (int*)xcalloc(train.X.rows, sizeof(int));
    int * test_nums = (int*)xcalloc(test.X.rows, sizeof(int));

    // Compteur du nombre d'images dans train et test
    int cpt_train = 0;
    int cpt_test = 0;

    // Remplit les matrices de train et test
    for(i = 0; i < d.X.rows; ++i){
        int num_patient = get_num_patient(d.filenames[i]);

        if(is_in(num_patient, train_nums, train.X.rows)){
            train.X.vals[cpt_train] = d.X.vals[i];
            train.y.vals[cpt_train] = d.y.vals[i];
            train_nums[cpt_train] = num_patient; //doublons.....
            cpt_train++;
        }
        else{
            if(is_in(num_patient, test_nums, test.X.rows)){
                test.X.vals[cpt_test] = d.X.vals[i];
                test.y.vals[cpt_test] = d.y.vals[i];
                test_nums[cpt_test] = num_patient; //doublons.....
                cpt_test++;
            }
            else{ //pb si pas le bon nombre dans chaque ensemble......
                if(cpt_train <= train.X.rows){
                    train.X.vals[cpt_train] = d.X.vals[i];
                    train.y.vals[cpt_train] = d.y.vals[i];
                    train_nums[cpt_train] = num_patient; //doublons.....
                    cpt_train++;
    ...
    //Stock les ensembles d'entraînement et de test dans le tableau de données de sortie
    split[0] = train;
    split[1] = test;
